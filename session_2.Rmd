...
# I edited some of the source code so that I could change some aspects of 
# these Venn diagrams I didn't like.  Below, I load in that modified function.
# I'll make this publicly available either with the rest of this tutorial, or 
# shortly afterwards.  You can run this with the base package, but the function
# is plotVenn3d(), and get rid of the printvals argument.
...
sr.rel.att.mess <- aov(vmessage ~ relprime*attitude, data=srp)
summary(sr.rel.att.mess) #main effect of attitude (interaction ns)
```
...
#### Type II SS
Type II resolves this issue, in a fashion.  It gives us the estimates, while controlling for the other variable.  That is:  
...
To do a type II in R, we should use the `car` package.  There ~~might be~~ is a way to do it without, but I don't know it, it's probably a bit more complicated:

```{r}
library(car)
sr.rel.att.mess <- aov(vmessage ~ attitude * relprime, data=srp)
summary(sr.rel.att.mess)
